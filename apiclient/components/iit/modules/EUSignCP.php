...
class EUSignCP
{
    ...
    private function resultToErrorCode($result, $errorCode)
    {
        switch ($result)
        {
...
//--------------------------------------------------------------------------------
    ...
    public function develop($envelopedData, &$data, &$senderInfo)
    {
        ...
        $result = euspe_develop(
            ...
            $senderInfo->signTime,
            $senderInfo->useTSP,
            $senderInfo->signerInfo->issuer,
            $senderInfo->signerInfo->issuerCN,
            $senderInfo->signerInfo->serial,
            $senderInfo->signerInfo->subject,
            $senderInfo->signerInfo->subjCN,
            $senderInfo->signerInfo->subjOrg,
            $senderInfo->signerInfo->subjOrgUnit,
            $senderInfo->signerInfo->subjTitle,
            $senderInfo->signerInfo->subjState,
            $senderInfo->signerInfo->subjLocality,
            $senderInfo->signerInfo->subjFullName,
            $senderInfo->signerInfo->subjAddress,
            $senderInfo->signerInfo->subjPhone,
            $senderInfo->signerInfo->subjEMail,
            $senderInfo->signerInfo->subjDNS,
            $senderInfo->signerInfo->subjEDRPOUCode,
            $senderInfo->signerInfo->subjDRFOCode,
            $errorCode
        );
        if ($result != EUSignCP::EM_RESULT_OK) {
            $errorCode = $this->resultToErrorCode($result, $errorCode);
